...
class SecuenciaCaracteres{
	...
	// v5 Elimina caracteres repetidos NO ACABADO. SIN LLEVAR A OTRO ARRAY NO, PERO ELIMINANDO DEL MISMO SI -.-
	void EliminaRepetidos(){
		...
		for(int a = 0; a < total_utilizados; a++){
			for(int b = a+1 ; b < total_utilizados; b++){
				if(vector_privado[a] != vector_privado[b]){
					vector_local[a]=vector_privado[a];
				}
			}
			usados++;
		}
		...
		for (int i=total_utilizados; i >= usados; i--){
			Elimina(i);
		}
	}
};
